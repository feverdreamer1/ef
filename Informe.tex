...
\begin{figure}[H]
  ...
  \label{fig:TsuEsaki}
\end{figure}
Sin embargo, en nuestra simulación, las resonancias del estado fundamental y excitado aparecen desplazadas hacia energías mayores ($\approx 0.125\text{ eV}$ y $\approx 0.5\text{ eV}$) en comparación con los resultados de su artículo original ($\approx 0.1\text{ eV}$ y $\approx 0.32\text{ eV}$) para N=2. Esta discrepancia no constituye un error, sino una mejora sustancial en la precisión del cálculo por dos motivos físicos fundamentales \cite{tsu_resonant2011}:
